...
make_config() {
    ...
    echo "socket='$mysqld_tmp_socket'" >>$config
...
su mysql -s /bin/bash -c "$mysqld_command --defaults-file=$mysqld_config \
    --skip-networking --socket=$mysqld_tmp_socket" &
parent=$!
...
for f in "${list[@]}" ; do
    ...
    case "$f" in
        ...
        *.sql) echo "$0: running $f";
            $mysql_command --defaults-file=$config $defaults_extra_file $no_defaults $argsc < $f;
            echo ;;
